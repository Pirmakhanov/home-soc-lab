...
```
home-soc-lab/
├── README.md       — Английская версия отчёта по проекту
├── README.ru.md    — Русская версия
├── LICENSE         — MIT License
└── images/         — Скриншоты Wazuh, Sysmon и разбора алертов
```
...
- Открыл дашборд по адресу `https://<wazuh-server-ip>` через дефолтные учётные данные лаба (для любого non-lab развёртывания их обязательно нужно ротировать)
...
*MITRE ATT&CK Dashboard — после симуляции 7 атак подсветились 6 тактик. Видно распределение алертов и временной график их возникновения.*

![Threat Hunting Events](images/03-threat-hunting.png)
*Threat Hunting view — 58 алертов за 30-минутное окно с фильтром `agent.name: Win11-Victim`. Видны конкретные правила: encoded PowerShell (level 12), file dropped (level 15), service creation (level 5), discovery activity (level 3).*

![Wazuh Dashboard](images/01-wazuh-dashboard.png)
*Wazuh Endpoints Summary — агент Win11-Victim в статусе Active, версия 4.14.5.*
...
*Раскрытый JSON алерта Level 12. Видны ключевые поля: `commandLine` с флагом `-EncodedCommand` и base64-блобом, `integrityLevel: High`, `user: socadmin`, `parentProcessGuid` (родитель — тоже PowerShell), `hashes` с SHA256 для проверки в TI-фидах.*
...
*Раскрытый JSON алерта Level 15. Ключевое поле — `targetFilename` со значением `__PSScriptPolicyTest_szs4ioxw.j3i.ps1`. Обратите внимание: `processGuid` совпадает с `ProcessGuid` из алерта Level 12 выше — это значит файл создал тот же PowerShell-процесс, через 1 секунду после запуска.*
...
**Пирмаханов Ерлан** — студент 4 курса по специальности «Информационная безопасность» в Satbayev University (Алматы), GPA 3.68 / 4.0. Открыт к стажировкам и junior-позициям в SOC.
...
📄 [Резюме](https://hh.kz/resume/06a9299fff10757f930039ed1f79525a396257)
